...
/**
 * Create a Jetty server.  This code is only used when running in the shaded
 * "uber-jar"; not when running from, for example, `mvn jetty:run`.
 */
public class WebServer
{
        ...
        throws Exception
    {
        ...
        // for when running as an uber-jar. Note that when running with
        // `mvn jetty:run`, the defaults are set in the pom.xml file.
        ...
        try {
            ...
            System.err.println("Failed to start web server; there's nothing " +
                "we can do! " + e.getMessage());
            System.exit(1);
        }
    }
    ...
    public static void initLogs() {
        ...
        requestLogPath = new File(System.getProperty("log"),
            "access/yyyy_mm_dd.request.log");
    }
    ...
    private void start() throws Exception
    {
        ...
        /*
         * Instantiate and configure an org.eclipse.jetty.server.Server object
        ...
        // org.eclipse.jetty.webapp.WebAppContext is one kind of Handler.
        ...
        // I think that wrapping the webAppContext in a HandlerList is because,
        // in general, a Jetty server should be able to serve more than one
        // webapp.  Of course, we are just serving one, so I think this is
        // unnecessary.
        List<Handler> handlers = new ArrayList<Handler>();
        handlers.add(webAppContext);
        HandlerList contexts = new HandlerList();
        // The argument to toArray specifies the type of the returned array,
        // i.e. Hander[]
        ...
        // A HandlerCollection is a Handler that passes a request to each of
        // its contained Handlers, in list order.  It differs from a HandlerList
        ...
        // Do not send the HTTP `Server` header, with the server version --
        // considered a security issue.
        for (Connector y : server.getConnectors()) {
            for (ConnectionFactory x  : y.getConnectionFactories()) {
                if (x instanceof HttpConnectionFactory) {
                    log.debug("Setting HttpConnectionFactory." +
    ...
    private String getShadedWarUrl() {
        String urlStr = Thread.currentThread().getContextClassLoader()
                .getResource(JETTY_XML).toString();
        // Strip off "WEB-INF/web.xml"
        return urlStr.substring(0, urlStr.length() - JETTY_XML.length());
    }
